...
class DrFuzz:
    ...
    def run(self):
        ...
        for i in range(len(I_label)):
            # if i in good_idx[0]:
              ...
              self.corpus += 1
        _, regression_faults_in_initial_seeds, rest_list, triggering_faults_in_initial_seeds, both_initial_faults,bf_i_l, rf_i_l, tf_i_l = self.experiment.coverage.initial_seed_list(I_list)
        # print("LLLLLLLLLLLLLLLLL", len(triggering_faults_in_initial_seeds))
        self.both_failure_case.extend(both_initial_faults)
        self.bf_l.extend(bf_i_l)
        self.triggering_failure.extend(triggering_faults_in_initial_seeds)
        self.tf_l.extend(tf_i_l)
        self.regression_faults.extend(regression_faults_in_initial_seeds)
        self.rf_l.extend(rf_i_l)
        self.bf_l.extend(bf_i_l)
        ...
        while not self.experiment.termination_condition():
            ...
            if len(time_list) > 0:
                if timing(starttime, time_list[0]): 
                    ...
                    # f.writelines('\nKKKKK',str(kk))
                    # f.writelines('KKKKK',kk)
                    # f.writelines('\nTOTAL ALL M1!= M2:' + str(All_F))
                    ...
                    # print("KKKKK",kk)
                    print("triggering_initial", triggering_faults_in_initial_seeds)
            ...
            if len(Mutants) > 0:
                bflist, rflist, wklist, hwklist, B_new, dangerous_source_id, fixlist , tflist, bflab, tflab, rflab = self.isFailedTestList(I, Mutants)
                self.triggering_failure.extend(tflist)
                count_triggering.extend(tflist)
                self.tf_l.extend(tflab)
                self.both_failure_case.extend(bflist)
                count_both_fail.extend(bflist)
                self.bf_l.extend(bflab)
                self.regression_faults.extend(rflist)
                count_regression.extend(rflist)
                self.rf_l.extend(rflab)
            ...
            All_F=kk+ len(triggering_faults_in_initial_seeds)

        return self.both_failure_case, self.regression_faults, self.weaken_case, self.triggering_failure, self.bf_l, self.tf_l,self.rf_l
    ...
    def isFailedTest(self, I_new):
        ...
        if predict_result_v1 == ground_truth and predict_result_v2 != ground_truth:
            return 2
        if predict_result_v1 == ground_truth and predict_result_v2 == ground_truth and difference > 0.3:
            return 3
        elif predict_result_v1 == ground_truth and predict_result_v2 == ground_truth and difference > 0.15:
            return 4
        #new I change
        if predict_result_v1 != predict_result_v2 :
            return 5
        else:
            return 0
    ...
    def isFailedTestList(self, I, I_new_list):
        global kk
        # kk=len(triggering_faults_in_initial_seeds)
        # print("KKKKKK",kk)
        model_v1 = self.experiment.model
        ...
        regression_faults_labels=[]
        weaken_faults_list = []
        half_weaken_faults_list = []
        rest_case_list = []
        fix_case_list = []
        potential_source_id = []
        triggering_faults_list=[]
        triggering_faults_labels=[]
        for i in range(len(I_new_list)):
            ...
            if predict_result_v1[i] != ground_truth_list[i] and predict_result_v2[i] != ground_truth_list[i]:
                ...
                both_file_labels.append([predict_result_v1[i], predict_result_v2[i], ground_truth_list[i]])
                print(both_file_list[0].ref_image)
                # plt.figure(figsize=(10, 10))
                # plt.imshow(both_file_list.ref_image, cmap='gray')
                # plt.title(f"GT: {ground_truth_label}, M1: {m1_label}, M2: {m2_label}")
                # plt.axis('off')
                # plt.savefig(f"/content/drive/MyDrive/Paper_Diff_testing/DRFuzz-main/images/Inital_triggering/Test_Case_MNIST_LeNet5{i}.png")
                # plt.show()
                # plt.close()
            if predict_result_v1[i] != predict_result_v2[i]:
                triggering_faults_list.append(I_new_list[i])
                triggering_faults_labels.append([predict_result_v1[i], predict_result_v2[i], ground_truth_list[i]])
                kk=kk+1
            if predict_result_v1[i] == ground_truth_list[i] and predict_result_v2[i] != ground_truth_list[i]:
                ...
                regression_faults_labels.append([predict_result_v1[i], predict_result_v2[i], ground_truth_list[i]])
        ...
        return both_file_list, regression_faults_list, weaken_faults_list, half_weaken_faults_list, rest_case_list, potential_source_id, fix_case_list , triggering_faults_list, both_file_labels, triggering_faults_labels, regression_faults_labels
